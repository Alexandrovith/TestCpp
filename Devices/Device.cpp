...
namespace TestCpp
{
	...
	{
		LoadDLL = new CLoadDLL (cpDll);

		DInitDll InitDll = (DInitDll)LoadDLL->Load ("InitDll");
		if (InitDll == nullptr)
			throw gcnew System::Exception ("Error DllLoad");
		InitDll ();

		if (LoadDLL->LoadFuncFromExtDll () == 1)
			throw gcnew System::Exception ("Error load func dll");
	}
	//_____________________________________________________________________________
	...
	int CDevice::RunSubscribe (System::Collections::Hashtable^ TagList)
	{
		try
		{
			for each (System::Collections::DictionaryEntry  tag in TagList)
			{
				Subscribe ((int)tag.Key, (char*)CConvMarsh::Convert (tag.Value->ToString()).c_str ());
			}
		}
		catch (const std::exception& exc)
		{			
			//OutMess (msclr::interop::marshal_as<System::String^> (exc.what ()));
			OutMess ("RunSubscribe: " + CConvMarsh::Convert (exc.what ()));
		}
		return 0;
	}
	//_____________________________________________________________________________

	void CDevice::CloseDev()
	{
		Close ();
	}
	//_____________________________________________________________________________
